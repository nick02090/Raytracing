...
namespace raytracing
{
   ...
   public class Picture : Form
   {
      ...
      public Picture ()
      {
         ...
         FileReader fileReader = new FileReader(@"C:\Users\Nikola\Documents\GitHub\Raytracing\Input.txt"); //change input path location
         fileReader.read();
      ...
      void Picture_Paint ( object sender, PaintEventArgs e )
      {
         for(int j = 0; j < screenResolution; j++)
         {
            for(int i = 0; i < screenResolution; i++)
            {
               ...
               Color c = Color.FromArgb(Convert.ToInt32(255 * colors.getRed()),
                   Convert.ToInt32(255 * colors.getGreen()),
                   Convert.ToInt32(255 * colors.getBlue()));
