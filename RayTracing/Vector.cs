...
namespace raytracing
{
    ...
    public class Vector
    {
        ...
        {
            // R = N * (2 * U*N) - U
            return normal.multiple(2 * dotProduct(normal)).sub(this);
        }
        ...
        public Vector getRefractedVector ( Vector normal, double nI )
        {
            // U
            normalize();
            var alpha = getAngle(normal);
            // D = 4 * (b^2 * cos(alpha)^2 - b^2 + 1)
            var D = 4 * (nI * nI * Math.Cos(alpha) * Math.Cos(alpha) - nI * nI  + 1);
            // a = (-2 * b * cos(alpha) + sqrt(D)) / 2
            var a = (-2 * nI * Math.Cos(alpha) + Math.Sqrt(D)) / 2;
            // R = -a*N - b*U
            return normal.multiple(-a).sub(multiple(nI));
        }
    }
}